...
func (j JobStore) Claim(ctx context.Context, now time.Time, lease time.Duration) (string, string, string, error) {
	...
	e = tx.QueryRowContext(ctx, `SELECT id,kind,payload FROM jobs WHERE status='pending' AND run_after<=? ORDER BY run_after LIMIT 1`, now.Format(time.RFC3339Nano)).Scan(&id, &kind, &payload)
	if e != nil {
		return "", "", "", e
	}
	until := now.Add(lease).Format(time.RFC3339Nano)
	if _, e = tx.ExecContext(ctx, `UPDATE jobs SET status='running',lease_until=?,updated_at=? WHERE id=? AND status='pending'`, until, now.Format(time.RFC3339Nano), id); e != nil {
		return "", "", "", e
	}
