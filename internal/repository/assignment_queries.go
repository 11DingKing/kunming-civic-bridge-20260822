...
func (a AssignmentQueryStore) Complete(ctx context.Context, id, owner string, now time.Time) error {
	res, e := a.DB.ExecContext(ctx, `UPDATE assignments SET status='done',completed_at=?,lease_token=NULL,lease_until=NULL,version=version+1 WHERE id=? AND status='claimed' AND lease_until>?`, now.Format(time.RFC3339Nano), id, now.Format(time.RFC3339Nano))
	if e != nil {
		return e
	}
	n, _ := res.RowsAffected()
