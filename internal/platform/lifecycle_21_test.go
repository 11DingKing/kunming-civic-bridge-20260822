...
func TestJSONCodecValidationMatrix(t *testing.T) {
	cases := []struct {
		name    string
		value   JSONCodec
		wantErr bool
	}{{"missing", JSONCodec{}, true}, {"negative", JSONCodec{Name: "x", Timeout: -time.Second}, true}, {"disabled", JSONCodec{Name: "x", Timeout: time.Second}, false}, {"ready", JSONCodec{Name: "x", Enabled: true, Timeout: time.Second}, false}}
	...
	x := JSONCodec{Name: "initial", Enabled: true, Timeout: time.Second}
	if !x.Ready() {
