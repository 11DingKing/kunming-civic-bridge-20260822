...
func (s ReopenService) Apply(ctx context.Context, id string) error {
	...
	_, e = s.DB.ExecContext(ctx, `UPDATE suggestions SET status='reopened',version=version+1,updated_at=? WHERE id=? AND status IN ('responded','closed')`, now.Format(time.RFC3339Nano), id)
	return e
}
